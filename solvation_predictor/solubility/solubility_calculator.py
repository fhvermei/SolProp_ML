...
class SolubilityCalculations:
    ...
    def __init__(
        ...
        self.unc_E, self.unc_S, self.unc_A, self.unc_B, self.unc_L = (
            None,
            None,
            None,
            None,
        ...
        self.logs_T_with_const_hdiss_from_aq, self.logs_T_with_T_dep_hdiss_from_aq = (
            None,
            None,
        )
        (
            self.logs_T_with_const_hdiss_warning_message,
            self.logs_T_with_T_dep_hdiss_error_message,
        ) = (None, None)
        self.hsolv_T, self.gsolv_T, self.ssolv_T = None, None, None
        self.logs_T_with_const_hdiss_from_ref, self.logs_T_with_T_dep_hdiss_from_ref = (
            None,
            None,
        )
    ...
    def make_calculations_298(
        self,
        predictions: SolubilityPredictions,
        calculate_aqueous: bool = None,
        calculate_reference_solvents: bool = None,
        verbose=False,
    ):
        ...
        self.unc_logk_298 = self.calculate_logk(
            gsolv=self.unc_gsolv_298, uncertainty=True
        )
        ...
        if calculate_aqueous:
            ...
            self.gsolv_aq_298, self.unc_gsolv_aq_298 = self.extract_predictions(
                predictions.gsolv_aq
            )
            self.logk_aq_298 = self.calculate_logk(gsolv=self.gsolv_aq_298)
            self.unc_logk_aq_298 = self.calculate_logk(
                gsolv=self.unc_gsolv_aq_298, uncertainty=True
            )

            self.logs_aq_298, self.unc_logs_aq_298 = self.extract_predictions(
                predictions.saq
            )
            self.logs_298_from_aq = self.calculate_logs_298(
                logk=self.logk_298, logk_ref=self.logk_aq_298, logs_ref=self.logs_aq_298
            )
            self.unc_logs_298_from_aq = self.calculate_logs_298(
                logk=self.unc_logk_298,
        ...
        if calculate_reference_solvents:
            ...
            self.unc_logk_ref_298 = self.calculate_logk(
                gsolv=self.unc_gsolv_ref_298, uncertainty=True
            )

            self.logs_ref_298 = np.array(predictions.data.reference_solubility)
            self.logs_298_from_ref = self.calculate_logs_298(
                logk=self.logk_298,
                logk_ref=self.logk_ref_298,
                logs_ref=self.logs_ref_298,
            )
            self.unc_logs_298_from_ref = self.calculate_logs_298(
                logk=self.unc_logk_298,
    ...
    def make_calculations_t(
        ...
        if predictions.solute_parameters:
            self.E, self.S, self.A, self.B, self.L = self.get_solute_parameters(
                predictions.solute_parameters[0]
            )
            (
                self.unc_E,
                self.unc_S,
                ...
                self.unc_B,
                self.unc_L,
            ) = self.get_solute_parameters(predictions.solute_parameters[1])

        self.V = np.array(
            [
                self.calculate_solute_parameter_v(sm[1])
                for sm in predictions.data.smiles_pairs
            ]
        )
        self.I_OHadj, self.I_OHnonadj, self.I_NH = self.get_diol_amine_ids(
            predictions.data.smiles_pairs
        )
        if self.hsubl_298 is None:
            self.hsubl_298 = self.get_hsubl_298(
                self.E,
                self.S,
                self.A,
                self.B,
        ...
        self.logs_T_with_const_hdiss_warning_message = (
            self.get_logs_t_with_const_hdiss_warning_message(
        ...
        if calculate_t_dep_with_t_dep_hdiss:
            if self.Cp_solid is None:
                self.Cp_solid = self.get_Cp_solid(
                    self.E, self.S, self.A, self.B, self.V, I_OHnonadj=self.I_OHnonadj
                )  # in cal/mol/K
            if self.Cp_gas is None:
                self.Cp_gas = self.get_Cp_gas(
                    self.E, self.S, self.A, self.B, self.V
                )  # in cal/mol/K
            ...
            if self.solv_crit_prop_dict is None:
                ...
                crit_data_path = io.BytesIO(
                    pkgutil.get_data("solvation_predictor", path)
                )
                self.solv_crit_prop_dict = json.load(
                    crit_data_path
                )  # inchi with fixed H is used as a solvent key

            coolprop_name_list, crit_t_list, crit_d_list = self.get_solvent_info(
                predictions.data.smiles_pairs, self.solv_crit_prop_dict
            )
        ...
        if calculate_aqueous:
            ...
            self.logs_T_with_const_hdiss_from_aq = self.calculate_logs_t(
                hsolv_298=self.hsolv_298,
            ...
            if calculate_t_dep_with_t_dep_hdiss:
                ...
                (
                    self.logs_T_with_T_dep_hdiss_from_aq,
                    self.logs_T_with_T_dep_hdiss_error_message,
                    self.hsolv_T,
                    self.gsolv_T,
                    self.ssolv_T,
                ) = self.calculate_logs_t_with_t_dep_hdiss_all(
                    gsolv_298_list=self.gsolv_298,
                    hsolv_298_list=self.hsolv_298,
                    hsubl_298_list=self.hsubl_298,
                    Cp_solid_list=self.Cp_solid,
                    Cp_gas_list=self.Cp_gas,
                    logs_298_list=self.logs_298_from_aq,
                    T_list=predictions.data.temperatures,
                    coolprop_name_list=coolprop_name_list,
                    Tc_list=crit_t_list,
                    rho_c_list=crit_d_list,
                )
        ...
        if calculate_reference_solvents:
            ...
            self.logs_T_with_const_hdiss_from_ref = self.calculate_logs_t(
                hsolv_298=self.hsolv_298,
                hsubl_298=self.hsubl_298,
            ...
            if calculate_t_dep_with_t_dep_hdiss:
                ...
                (
                    self.logs_T_with_T_dep_hdiss_from_ref,
                    self.logs_T_with_T_dep_hdiss_error_message,
                    self.hsolv_T,
                    self.gsolv_T,
                    self.ssolv_T,
                    ...
                    Cp_solid_list=self.Cp_solid,
                    Cp_gas_list=self.Cp_gas,
                    logs_298_list=self.logs_298_from_ref,
                    T_list=predictions.data.temperatures,
                    coolprop_name_list=coolprop_name_list,
                    Tc_list=crit_t_list,
                    rho_c_list=crit_d_list,
    ...
    def extract_predictions(self, predictions):
        ...
        unc = (
            np.sqrt(np.array(predictions[1])) if predictions else None
        )  # uncertainty reported as standard deviation
        return pred, unc

    def calculate_logs_t(
        self, hsolv_298=None, hsubl_298=None, logs_298=None, temperatures=None
    ):
        hdiss_298 = hsolv_298 + hsubl_298
        return logs_298 - hdiss_298 / 2.303 / 8.314 * 4.184 * 1000 * (
            1 / temperatures - 1 / 298.0
        )

    def get_logs_t_with_const_hdiss_warning_message(self, temperatures=None):
        warning_message = [
            "Warning! Above 350 K, `calculate_t_dep_with_t_dep_hdiss` option is recommended."
            if temp > 350
            else ""
    ...
    def calculate_logs_298(
        self, logk=None, logk_ref=None, logs_ref=None, uncertainty: bool = False
    ):
        if uncertainty:
    ...
    def calculate_logk(self, gsolv=None, uncertainty: bool = False):
        if uncertainty:
            return np.abs(gsolv * 4.184 * 1000.0 / 8.314 / 298.0 / 2.303)
        else:
            return -gsolv * 4.184 * 1000.0 / 8.314 / 298.0 / 2.303  # in log10
    ...
    def get_hsubl_298(
        self, E, S, A, B, V, I_OHadj=None, I_OHnonadj=None, I_NH=None, in_kcal=True
    ):
        """
    ...
    def calculate_solute_parameter_v(self, solute_smiles):
        ...
        for atom in mol.GetAtoms():
            try:
                ...
                raise ValueError(
                    "McGowan volume not available for element {}".format(
                        atom.GetAtomicNum()
                    )
                )
    ...
    def check_valid_t(self, T, Tc, coolprop_name=None, ref_solvent="n-Heptane"):
        ...
        if T > T_max:
            error_message = (
                f"Temperature {T} K is above the critical temperature {T_max} K."
            )
            valid = False
        elif T > T_max - 15:
    ...
    def get_gas_liq_sat_density(
        self, T, Tc, rho_c, coolprop_name=None, ref_solvent="n-Heptane"
    ):
        if coolprop_name is None:
            return self.get_gas_liq_sat_density_from_ref(
                T, Tc, rho_c, ref_solvent=ref_solvent
            )
        else:
    ...
    def get_gas_liq_sat_density_from_ref(self, T, Tc, rho_c, ref_solvent="n-Heptane"):
        ...
        gas_density_ref, liq_density_ref = self.get_gas_liq_sat_density_from_cp(
            T_ref, ref_solvent
        )
    ...
    def get_Kfactor_parameters(
        self, gsolv_298, hsolv_298, Tc, rho_c, coolprop_name, T_trans_factor=0.75
    ):
        T1 = 298
        ...
        T_transition = (
            Tc * T_trans_factor
        )  # T_trans_factor is empirically set to 0.75 by default
        ...
        rho_g_298, rho_l_298 = self.get_gas_liq_sat_density(
            T1, Tc, rho_c, coolprop_name=coolprop_name
        )
        ...
        (
            rho_g_T2,
            rho_l_T2,
        ) = self.get_gas_liq_sat_density(T2, Tc, rho_c, coolprop_name=coolprop_name)
        ...
        Amatrix[1][2] = (
            ...
            / Tc
            * np.exp(1 - T1 / Tc)
            * (0.59 * (T1 / Tc) ** (-0.41) - (T1 / Tc) ** 0.59)
        )
        Amatrix[1][3] = 0
        bvec[1] = slope298
        # 3. Tr*ln(K-factor) continuity at T = T_transition
        rho_g_Ttran, rho_l_Ttran = self.get_gas_liq_sat_density(
            T_transition, Tc, rho_c, coolprop_name=coolprop_name
        )
        Amatrix[2][0] = 1
        ...
        T3 = T_transition + 1
        rho_g_T3, rho_l_T3 = self.get_gas_liq_sat_density(
            T3, Tc, rho_c, coolprop_name=coolprop_name
        )
        Amatrix[3][0] = 0
    ...
    def get_t_dep_gsolv_kfactor(
        self, T, Tc, rho_c, coolprop_name, kfactor_parameters, only_gsolv=True
    ):
        ...
        rho_g, rho_l = self.get_gas_liq_sat_density(
            T, Tc, rho_c, coolprop_name=coolprop_name
        )
        if T < T_transition:
            kfactor = np.exp(
                (
    ...
    def get_t_dep_hsolv(
        self, T, Tc, rho_c, coolprop_name, kfactor_parameters, T_min, T_max
    ):
        ...
        if T - 1 < T_min:
            ...
            T_list = [T - 1, T, T + 1]
        gsolv_list = [
            self.get_t_dep_gsolv_kfactor(
                T, Tc, rho_c, coolprop_name, kfactor_parameters
            )
            for T in T_list
        ]
    ...
    def hsolv_integrand(
        ...
        hsolv_T, _, _ = self.get_t_dep_hsolv(
            T, Tc, rho_c, coolprop_name, kfactor_parameters, T_min, T_max
        )  # in kcal/mol
        integrand = hsolv_T * 4.184 * 1000.0 / (8.314 * T**2)
        return integrand

    def integrate_hsolv_term(
        self,
        T,
        const_hdiss_T,
        Tc,
        ...
        if const_hdiss_T is not None:
            ...
            hsolv_integral += (
                (-hsolv_T * 4.184 * 1000.0) / 8.314 * (1 / T - 1 / const_hdiss_T)
            )
    ...
    def integrate_t_dep_hdiss(self, hsolv_integral, hsubl_298, Cp_solid, Cp_gas, T):
        ...
        total_integral = (
            hsolv_integral + hsub_integral + Cpgas_integral + Cpsolid_integral
        )
        return total_integral

    def calculate_logs_t_with_t_dep_hdiss(
        self,
        gsolv_298=None,
        hsolv_298=None,
        hsubl_298=None,
        ...
        valid, const_hdiss_T, error_message, T_min, T_max = self.check_valid_t(
            T, Tc, coolprop_name=coolprop_name
        )
        ...
        kfactor_parameters = self.get_Kfactor_parameters(
            gsolv_298, hsolv_298, Tc, rho_c, coolprop_name
        )
        hsolv_integral = self.integrate_hsolv_term(
            T, const_hdiss_T, Tc, rho_c, coolprop_name, kfactor_parameters, T_min, T_max
        )
        total_integral = self.integrate_t_dep_hdiss(
            hsolv_integral, hsubl_298, Cp_solid, Cp_gas, T
        )
        logs_t = logs_298 + total_integral / 2.303
        # get hsolv_T
        if const_hdiss_T is None:
            hsolv_T, gsolv_T, ssolv_T = self.get_t_dep_hsolv(
                T, Tc, rho_c, coolprop_name, kfactor_parameters, T_min, T_max
            )
        else:
            hsolv_T, gsolv_T, ssolv_T = self.get_t_dep_hsolv(
    ...
    def calculate_logs_t_with_t_dep_hdiss_all(
        self,
        gsolv_298_list=None,
        hsolv_298_list=None,
        hsubl_298_list=None,
        ...
        for i in range(len(gsolv_298_list)):
            if Tc_list[i] is None or rho_c_list is None:
                ...
                error_message_list.append(
                    "The given solvent is not supported. Its critical temperature and"
                ...
                (
                    ...
                    gsolv_T,
                    ssolv_T,
                ) = self.calculate_logs_t_with_t_dep_hdiss(
                    gsolv_298=gsolv_298_list[i],
                    hsolv_298=hsolv_298_list[i],
                    hsubl_298=hsubl_298_list[i],
                    Cp_solid=Cp_solid_list[i],
                    Cp_gas=Cp_gas_list[i],
                    logs_298=logs_298_list[i],
                    T=T_list[i],
                    coolprop_name=coolprop_name_list[i],
                    Tc=Tc_list[i],
                    rho_c=rho_c_list[i],
                )
