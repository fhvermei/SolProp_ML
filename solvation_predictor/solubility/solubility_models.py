...
class SolubilityModels:
    def __init__(
        ...
        if load_g or load_h or load_saq:
            self.g_models = (
                self.load_g_models(reduced_number=reduced_number, verbose=verbose)
                if load_g
                else None
            )
            self.h_models = (
                self.load_h_models(reduced_number=reduced_number, verbose=verbose)
                if load_h
                else None
            )
            self.saq_models = (
                self.load_saq_models(reduced_number=reduced_number, verbose=verbose)
                if load_saq
                else None
            )
    ...
    def load_g_models(self, reduced_number=False, verbose=True):
        ...
        paths = [
            os.path.join("trained_models", "Gsolv", "model_Gsolv_" + str(i) + ".pt")
    ...
    def load_h_models(self, reduced_number=False, verbose=True):
        ...
        paths = [
            os.path.join("trained_models", "Hsolv", "model_Hsolv_" + str(i) + ".pt")
            for i in range(number)
        ]
        if verbose:
    ...
    def load_saq_models(self, reduced_number=False, verbose=True):
        ...
        paths = [
            os.path.join("trained_models", "Saq", "model_" + str(i) + ".pt")
            for i in range(number)
        ]
        if verbose:
