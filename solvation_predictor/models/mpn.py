...
class MPN(nn.Module):
    ...
    def __init__(
        ...
        if not self.atom_messages:
            self.W_i = nn.Linear(
                self.fb_size + self.fa_size, self.hidden_size, bias=self.bias
            )
            self.W_h = nn.Linear(self.hidden_size, self.hidden_size, bias=self.bias)
            self.W_o = nn.Linear(
                self.hidden_size + self.fa_size, self.hidden_size, bias=self.bias
            )
        else:
            self.W_i = nn.Linear(self.fa_size, self.hidden_size, bias=self.bias)
            self.W_h = nn.Linear(self.hidden_size, self.hidden_size, bias=self.bias)
            self.W_o = nn.Linear(
                self.hidden_size + self.fb_size, self.hidden_size, bias=self.bias
            )
